   ...
   public function cart(Request $request)
{
    ...
    if ($this->isAuth()) {
        ...
        // FIX: use actual session ID used for insert
        $sessionId = $request->session()->get('session_id') ?? $request->session()->getId();

        \Log::info('Guest Cart session ID: ' . $sessionId); // Debug log
   ...
   public function addToCart(Request $request)
{
    ...
    // For guest users: ensure a consistent session_id is used
    if ($customerId) {
        $sessionId = null;
    } else {
        if (!$request->session()->has('session_id')) {
            $request->session()->put('session_id', $request->session()->getId());
        }
